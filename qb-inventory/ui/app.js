...
// Shop
var currentcart = {}
Ax.Inventory.OpenShop = function(data){
    $('.shop-items').html('')
    $('.shop').fadeIn()
    $.each(data.items,function(k,v){
        $('.shop-items').prepend(`
        <div data-shopitem-name='${v.name}' class="shop-item">
            <div class="shop-item-header">${v.label}</div>
            <img src="items/${v.image}" class="shop-item-img">
            <div class="shop-item-price">
                <i class="fas fa-usd-circle"></i>${v.price}
            </div>
            <input placeholder='Amount' type=number oninput="validity.valid||(value='');" class="shop-item-add-to-cart-btn"></input>
        </div>
        `).find(`[data-shopitem-name='${v.name}'`).find('.shop-item-add-to-cart-btn').change(function(){
            currentcart[v.name] = {amount:$(this).val(),price:v.price}
        })
    })
    $('.shop-checkout').off().click(function(){
        $.post('https://qb-inventory/Checkout',JSON.stringify({items:currentcart}))
        currentcart = {}
        $('.shop-items').html('')
        $('.shop').fadeOut()
...
window.addEventListener('message', function(event) {
    switch(event.data.action) {
        ...
        case 'hotbar':
            ...
            break;
        case 'OpenShop':
            Ax.Inventory.OpenShop(event.data)
            break;
    }
})
